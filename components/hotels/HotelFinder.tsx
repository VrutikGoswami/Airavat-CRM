...
function websiteHotelSelection(enquiry?: Enquiry) {
  return parseCustomerHotelSelection(enquiry?.requirements ?? "");
}
...
function HotelImage({ hotel, className, url = hotel.imageUrls[0] }: { hotel: HotelMetadata; className: string; url?: string }) {
  return (
...
export function HotelFinder() {
  ...
  const initialHotelSelection = websiteHotelSelection(initial);
  ...
  const [rooms, setRooms] = useState(initialHotelSelection.rooms);
  ...
  const [customerSelectedRateId, setCustomerSelectedRateId] = useState<string | null>(null);
  ...
  const searchHotels = async () => {
    ...
    try {
      ...
      if (availabilityMode && body.results.length) {
        const matchedRateId = resolveCustomerSelectedRate(
          body.results,
          websiteHotelSelection(requestedEnquiry),
          body.nights,
        );
        setCustomerSelectedRateId(matchedRateId);
        const preferred = body.results.find((item) => item.rates.some((rate) => rate.id === matchedRateId));
...
function RoomRow({ hotel, rate, selected, preferred, onToggle }: { hotel: HotelMetadata; rate: HotelRateOffer; selected: boolean; preferred: boolean; onToggle: (hotel: HotelMetadata, rate: HotelRateOffer) => void }) {
  return <tr className={preferred ? "bg-info text-white [&_.text-muted]:text-white/80" : undefined}><td className="px-3 py-3 align-top"><p className="font-bold">{rate.roomType}</p>{preferred ? <span className="badge mt-1 border-white bg-white text-info">Customer selected</span> : <p className="mt-1 text-xs text-muted">{rate.rateType}</p>}</td><td className="px-3 py-3 align-top"><p>{rate.mealPlan}</p><p className="mt-1 text-xs text-muted">{rate.occupancy}</p></td><td className="px-3 py-3 align-top"><p>{rate.unitBasis}</p><p className="mt-1 text-xs text-muted">Tax {rate.taxIncluded.toLowerCase()} | Commission {rate.commissionIncluded.toLowerCase()}</p></td><td className="max-w-64 px-3 py-3 align-top"><p className="line-clamp-2 text-xs">{cancellationSummary(rate.cancellationPolicy)}</p><p className="mt-1 line-clamp-2 text-xs text-muted">{rate.paymentTerms || "Payment terms not supplied"}</p></td><td className="tnum px-3 py-3 text-right align-top">{formatMoney(rate.netTotal, rate.currency)}</td><td className="tnum px-3 py-3 text-right align-top font-bold">{formatMoney(rate.clientTotal, rate.currency)}</td><td className="px-3 py-3 text-right align-top"><button className={selected ? "btn border-white bg-white text-info" : "btn btn-primary hover:btn-primary-hover"} onClick={() => onToggle(hotel, rate)} disabled={rate.requiresConfirmation}>{selected ? <><Check className="size-4" /> Selected</> : rate.requiresConfirmation ? "Confirm basis" : "Select"}</button></td></tr>;
}
...
function RoomCard({ hotel, rate, rooms, selected, preferred, onToggle }: { hotel: HotelMetadata; rate: HotelRateOffer; rooms: number; selected: boolean; preferred: boolean; onToggle: (hotel: HotelMetadata, rate: HotelRateOffer) => void }) {
  return <div className={`rounded-lg border p-4 ${preferred ? "border-info bg-info text-white [&_.text-muted]:text-white/80" : "border-line"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-bold">{rate.roomType}</p>{preferred ? <span className="badge mt-1 border-white bg-white text-info">Customer selected</span> : null}<p className="text-sm text-muted">{rate.mealPlan} | {rate.occupancy}</p></div><p className="tnum text-right font-bold">{formatMoney(rate.clientTotal, rate.currency)}</p></div><dl className="mt-3 grid grid-cols-2 gap-2 text-xs"><div><dt className="text-muted">Rate basis</dt><dd>{rate.unitBasis}</dd></div><div><dt className="text-muted">Supplier rate</dt><dd className="tnum">{formatMoney(rate.netTotal, rate.currency)}</dd></div><div><dt className="text-muted">Tax</dt><dd>{rate.taxIncluded}</dd></div><div><dt className="text-muted">Pricing</dt><dd>{rate.pricingBasis === "rack" ? "Rack" : "Net"} + {HOTEL_MARKUP_PERCENT}%</dd></div></dl><p className="mt-3 text-xs">{cancellationSummary(rate.cancellationPolicy)}</p><p className="mt-1 text-xs text-muted">{rate.paymentTerms || "Payment terms not supplied"}</p><button className={selected ? "btn mt-4 w-full border-white bg-white text-info" : "btn btn-primary hover:btn-primary-hover mt-4 w-full"} onClick={() => onToggle(hotel, rate)} disabled={rate.requiresConfirmation}>{selected ? <><Check className="size-4" /> Selected</> : rate.requiresConfirmation ? "Confirm rate basis" : `Select for ${rooms} room${rooms === 1 ? "" : "s"}`}</button></div>;
}
